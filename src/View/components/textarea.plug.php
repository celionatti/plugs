<?php
/**
 * Built-in Textarea Component
 *
 * Usage: <x-textarea name="bio" label="Bio" rows="4" />
 *        <x-textarea name="message" label="Message" error="Required" />
 *
 * Props: name, label, error, rows, placeholder, value, required, disabled
 */
...
$inputId = $name ? 'textarea-' . htmlspecialchars($name) : 'textarea-' . substr(md5(uniqid()), 0, 8);
...
<div style="margin-bottom:16px" <?= $attributes ?? '' ?>>
    ...
    <textarea
        ...
        style="width:100%;padding:10px 14px;font-size:14px;line-height:1.6;color:#1e293b;background:<?= $disabled ? '#f8fafc' : '#fff' ?>;border:1.5px solid <?= $borderColor ?>;border-radius:10px;outline:none;transition:border-color .2s ease,box-shadow .2s ease;resize:vertical;font-family:inherit;box-sizing:border-box"
