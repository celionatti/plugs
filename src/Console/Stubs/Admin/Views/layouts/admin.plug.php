...
<head>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700&family=Dancing+Script:wght@400..700&display=swap"
        rel="stylesheet">
    ...
    <style>
        :root {
            --primary-color: {
                    {
                    \App\Models\Setting: :getValue('primary_color', '#6366f1')
                }
            }
            ...
            --secondary-color: {
                    {
                    \App\Models\Setting: :getValue('secondary_color', '#4f46e5')
                }
            }
            ...
            --border-radius: {
                    {
                    \App\Models\Setting: :getValue('border_radius', '1.5rem')
                }
            }
...
<body
    class="h-full text-slate-900 dark:text-slate-100 selection:bg-indigo-100 dark:selection:bg-indigo-900 selection:text-indigo-700 dark:selection:text-indigo-300 overflow-hidden bg-slate-50 dark:bg-slate-950">

    <div class="flex h-full">
        <!-- Sidebar -->
        <aside id="adminSidebar"
            class="fixed inset-y-0 left-0 z-50 w-72 bg-black border-r border-white/5 transition-transform duration-300 ease-in-out transform -translate-x-full lg:translate-x-0 lg:static lg:inset-0">
            <div class="flex flex-col h-full bg-black">
                <!-- Sidebar Header -->
                <div class="p-6">
                    <a href="{{ url('/') }}" class="flex items-center gap-3 group">
                        <div
                            class="w-10 h-10 gradient-bg rounded-xl flex items-center justify-center transition-transform group-hover:rotate-12">
                            <span class="text-white font-bold text-2xl brand-font">P</span>
                        </div>
                        <div class="flex flex-col">
                            <span
                                class="font-bold text-xl tracking-tight text-white brand-font leading-none">Plugs</span>
                            <span class="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-0.5">Admin
                                Central</span>
                        </div>
                    </a>
                </div>
                ...
                <nav class="flex-1 px-4 py-4 space-y-1.5 overflow-y-auto custom-scrollbar">
                    ...
                    <a href="/admin/migrations" data-spa="true"
                        class="sidebar-link flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-xl transition-all duration-200 text-slate-400 hover:text-white hover:bg-white/5 group">
                        <svg class="w-5 h-5 opacity-70 group-hover:text-white" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="/admin/themes" data-spa="true"
                        class="sidebar-link flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-xl transition-all duration-200 text-slate-400 hover:text-white hover:bg-white/5 group">
                        <svg class="w-5 h-5 opacity-70 group-hover:text-white" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="/admin/ai-prompt" data-spa="true"
                        class="sidebar-link flex items-center gap-3 px-4 py-3 text-sm font-semibold rounded-xl transition-all duration-200 text-slate-400 hover:text-white hover:bg-white/5 group">
                        <svg class="w-5 h-5 opacity-70 group-hover:text-white" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="p-4 border-t border-white/5">
                    <a href="/admin/profile" data-spa="true"
                        class="flex items-center gap-3 p-3 rounded-2xl bg-white/5 hover:bg-white/10 transition-colors group">
                        <div
                            class="w-10 h-10 rounded-full bg-white flex items-center justify-center text-black font-bold text-sm">
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
            <!-- Header -->
            <header
                class="h-20 glass sticky top-0 z-40 flex items-center justify-between px-8 border-b border-white/20 dark:border-white/5">
                <div class="flex items-center gap-4">
                    <button id="sidebarToggle"
                        class="lg:hidden p-2 rounded-xl bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all active:scale-95">
                    ...
                    <div class="hidden sm:block">
                        ...
                        <div class="flex items-center gap-2 mt-0.5">
                            ...
                            <div
                                class="px-2 py-0.5 rounded-full bg-black dark:bg-white text-white dark:text-black text-[10px] font-bold uppercase tracking-wider">
                                Active</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-4">
                    <button id="theme-toggle" type="button"
                        class="p-2.5 rounded-xl text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-slate-800 dark:hover:text-indigo-400 transition-all">
                    ...
                    <button
                        class="p-2.5 rounded-xl text-slate-400 hover:text-indigo-600 hover:bg-slate-100 dark:hover:bg-slate-800 dark:hover:text-indigo-400 transition-all">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <form action="{{ route('logout') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit"
                            class="px-5 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-sm font-bold hover:bg-rose-50 dark:hover:bg-rose-900/30 hover:text-rose-600 dark:hover:text-rose-400 transition-all active:scale-95">
                            Sign Out
                        </button>
                    </form>
                </div>
            ...
            <!-- Content Area -->
            <main-content class="flex-1 overflow-y-auto custom-scrollbar p-8" flash />
        </div>
    </div>
