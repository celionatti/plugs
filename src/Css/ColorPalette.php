...
/**
 * Complete OKLCH color palette for the Plugs CSS engine.
 *
 * 22 named colors × 11 shades (50–950), matching Tailwind's design system
 * but using the modern OKLCH color space for perceptual uniformity.
 */
class ColorPalette
{
    /**
    ...
    /**
     * Full color palette: name => [shade => oklch(...)]
     *
    ...
    /**
     * Custom user-defined colors merged at runtime.
     *
     * @var array<string, array<int, string>>
     */
    ...
    {
        return isset(self::$palette[$name])
            || isset(self::$customColors[$name])
            || isset(self::$keywords[$name]);
    }
    ...
    public static function registerCustomColors(array $colors): void
    {
        foreach ($colors as $name => $shades) {
            self::$customColors[$name] = $shades;
        }
    }

    /**
     * Get the keyword colors map.
     *
    ...
    /**
     * Reset custom colors (useful for testing).
     */
    public static function resetCustomColors(): void
    {
        self::$customColors = [];
    }
}
